...
const intsigMatch = core.matchScanned([intsig], [{
...
assert(core.statusChanged(intsig, intsigMatch[0].scanned), '状态变化应被检测');

const jdMatch = core.matchScanned([jd], [{
...
console.log('OfferTrack follow-up core tests: PASS');
console.log({ targets: targets.length, groups: groups.map(g => g.host), intsigScore: intsigMatch[0].score, jdScore: jdMatch[0].score });
