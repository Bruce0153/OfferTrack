...
const contractCode = fs.readFileSync(path.join(__dirname, '..', 'application-contract.js'), 'utf8');
const semanticCode = fs.readFileSync(path.join(__dirname, '..', 'semantic.js'), 'utf8');
...
vm.runInContext(semanticCode, context);
...
assert.equal(out[0].company, '星辰智造科技有限公司', 'central contract must preserve the stronger structured company identity');
const snap = context.__offerTrackSemanticSnapshot();
assert(!snap.companies.some(x => /旁观者/.test(x.value)), 'related-company noise must remain excluded');
